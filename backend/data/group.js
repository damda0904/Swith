...
export async function findGroupsByCK(category, keyword) {
    return StudyGroup.find({ category, title: keyword, content: keyword, recruit: true }, 'title, mainImage, description, startDate, EndDate, personnel, faceToFace, participant')
        .then(group => {
            console.log("group: " + group);
...
export async function findGroupsByKey(keyword) {
    return StudyGroup.find({ title: keyword, recruit: true }, 'title, mainImage, description, startDate, EndDate, personnel, faceToFace, participant')
        .then(group => {
            console.log("group: " + group);
...
export async function apply(id, userId) {
    return StudyGroup.findById(id)
        .then(group => {
            ...
            if (participants.length == personnel) {
                return -1
            } else if (participants.find(userId)) {
                return -2
            }
